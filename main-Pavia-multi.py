...
def train(network, train_data_loader, num_epochs, learning_rate, save_model):
    ...
    for epoch in tqdm(range(num_epochs), file=sys.stdout):
        ...
        band_attention = torch.mean(band_attention, dim=0).reshape(1, -1)
        if epoch % 5 == 0:
...
if __name__ == '__main__':
    ...
    data = np.transpose(or_data, (2,0,1))
    ...
    for test_num in test_set:
        ...
        for band_num in band_num_sets:
            ...
            np.save(f'res/band_selection/our_{dataset_name}_band_num_{band_num}_test_{test_num}.npy', selected_bands)
